        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <x-splade-table :for="$histories">
                        <x-slot:empty-state>
                            <p>No hay Historia Registrada! </p>
                        </x-slot:empty-state>
                        <x-splade-cell actions as="$history">
                        <Link 
                                modal 
                                href="{{ route('histories.show', $history->id) }}" 
                            ...
                            <Link 
                                modal 
                                href="{{ route('histories.edit', $history->id) }}" 
                                class="bg-blue-500 hover:bg-blue-700 flex px-4 py-2 rounded" 
                                title="Editar"> 
                                <x-pencil-icon class="w-5 h-5" /> 
                            </Link>
                            <x-splade-form action="{{ route('histories.destroy', $history) }}" class="flex px-2" title="Eliminar" method="DELETE" confirm>
