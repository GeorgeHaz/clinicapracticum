...
<h2 class="text-lg font-medium text-gray-900">
    ...
    <x-splade-form :default="$history" :action="route('histories.update', $history)" method="patch">
        <x-splade-select name="patient_id" :label="__('Paciente')">
            <option value="" disabled>{{ __('Seleccione un paciente') }}</option>
            @foreach ($patients as $patient)
            <option value="{{ $patient->id }}" @selected(old('patients_id', $history->patients_id) == $patient->id)>
        ...
        <x-splade-select name="doctor_id" :label="__('Médico')">
            ...
            <option value="{{ $doctor->id }}" @selected(old('doctors_id', $history->doctors_id) == $doctor->id)>
